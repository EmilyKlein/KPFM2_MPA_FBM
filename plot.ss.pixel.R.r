plot.ss.pixel.R<-function(ss.object, annual.plots=TRUE, connector="sum", page.layout = c(4, 4), Rylimits = c(0, 3))
{
  ...
  par(oma = c(4, 2, 4, 3), mar=c(4,4,1,0)+0.1, mgp=c(2,0.75,0), xpd=FALSE, mfrow = page.layout)
	...
	for(i in 1:ss.object$setup$nssmus) {
      ...
      abline(v=vlines,lty=3)
		...
		title(main=paste("Pixel ", title.list[i], sep = ""), outer = FALSE, cex.main = 0.9, line = 0.5)
