plot.ss.pixel.N<-function(ss.object, annual.plots=TRUE, connector="avg", page.layout = c(4, 4), Nylimits = c(0, 3))
{
	# auxiliary function to plot abundance from single run of predator-prey model
	# George Watters
	# code last edited 14 June 2006, updated for pixelated foosa June 2017 ESK
	#
  ...
  #
  # create list for plot titles - ESK June 2017
  ssmu.orig <- ss.object$setup$orig.SSMU
  ...
  origpar <- par(no.readonly=TRUE)
    ...
    lines(plot.time, c(seals.denom[i],seals.y[,i])/seals.denom[i], col = "red")
      ...
      abline(v=vlines,lty=3)
		...
		title(main=paste("Pixel ", title.list[i], sep = ""), outer = FALSE, line = 0.5, cex.main = 0.9)
