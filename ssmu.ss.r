ssmu.ss<-function(param.list, saved.state = NULL, GGP=NULL, nyears = 38, start.fishing.yr =1, stop.fishing.yr = NULL, rescale.catch = FALSE, random.Rkrill = FALSE, sd.krill.Rdev = 0.4,
  ...
  safe.mode = TRUE, annual.plots = TRUE, annual.connector = "default", Rylimits =	c(0, 3), Nylimits = c(0, 3), Cylimits = c(0, 3), page.layout = c(4, 4), point.size = 0.001, single.sim = TRUE, suppress.messages=FALSE, ss.seed = 123)
{
  # George Watters - last edited 2013
  # ESK edited this code to remove unnecessary comments and old code - May 2018
	#
...
### Basic set up is to here###
	...
	# IN THE PAST -- FOR NOW I ASSUME THAT IT'S JUST EQUAL TO OUR INITIALIZATION VALUES -- Area for future work
	#
  ...
  # earmarks is the initial, allocated catch for each ssmu
	...
	if(!is.null(fishing.option)) {
	  ...
	  ## and ESK added for option 7 - FBM related to penguins
    if(fishing.option == 1 | 7){
  ...
  for(sim.t in 1:ntimes) {
    ...
    #
	  # first thing to do is any reassessments (and reallocating catches) if necessary
		## NEW CALL HERE FOR allocate.catch.FBM -- update ESK 2018
...
#### NEW CODE FOR FBM ###
	      if(fishing.option == 7){
	        # last <- reassessment.interval-nseasons
	        # Get historical catches for period before assessment for years prior to reassessment if needed - catch last year, in same season
	        if(sim.t<reassessment.interval){use.me.catch<-historical.catch}else{
	          # below sums the catch in the last year over both seasons (could alteratively pull one season or use average)
	          use.me.catch <- colSums(catch[(sim.t-1):(sim.t-2),])
	          # or use catch as average of historical catches, i.e. as in fishing option 1
	          # use.me.catch <- colMeans(catch[1:sim.t-1,])
	          # or  catch as average ofcatches since last assessment 
	          # use.me.catch <- colMeans(catch[sim.t-reassessment.interval:sim.t-1,])
	        }
	        # use.me.catch <- catch[sim.t-reassessment.interval,]
	        # Also need change in penguins OR penguin density - update here depending on what we want to use
	        if(sim.t<=reassessment.interval){
	          # need this bit so we aren't referencing time step 0 later - can also update here so the first step is via fishing option 1
	          use.me.pengs <- pengs[sim.t,]
	          use.me.pengs <- use.me.pengs/sum(use.me.pengs)
	        }else{
	          # First pengs are for year after previous monitoring year (same season)
	          pengs.1 <- (pengs[sim.t-reassessment.interval,1:nssmus])
	          # Now penguins at correct time t for updating (same season, last year)
	          pengs.2 <- (pengs[sim.t-nseasons,1:nssmus])
	          # Relative difference between pengs in t-5 and t, pengs.2/pengs.1, or difference, pengs.2 - pengs.1
	          use.me.pengs <- pengs.2-pengs.1
	          # Density of penguins
	          # use.me.pengs <- (pengs.2/areas[1:15])/(pengs.1/areas[1:15])
	        }
	        if(all(use.me.catch==0)){use.me.catch<-historical.catch}
	        use.me.pengs[is.na(use.me.pengs)]<-0
	        tt <- allocate.catch(fishing.option=7, historical.catch=use.me.catch, init.tot.demand=use.me.pengs, init.krill.biomass=NULL, option5.list=NULL,
	                                 tt.density=NULL, relQdev1=NULL, seals.params.list=NULL, pengs.params.list=NULL, earmarks=earmarks, B0=B0.mult*sum(krill[krill.t-1, 1:nssmus] * wbar.vector.t[1:nssmus])*exp(B0.errors[tt.index]), actual.gamma=actual.gamma,
	                                 wbar.vector=wbar.vector.t[1:nssmus], catch.setup=catch.setup, ntimes=ntimes, nssmus=nssmus,
	                                 start.fishing=start.fishing, obs.errors=allocation.errors[tt.index,], obs.bias=allocation.mult)
	      }
##### END NEW CODE ####
    ...
    # loop over all areas to get mortalities and "competition reduction fractions"
      ...
      # use fishing thresholds to set catch = 0 if krill density is not "sufficient"
			if(i <= nssmus){
        if(krill.density[i] < threshold.density[i] && earmarks[sim.t,i] > 0){
          catch[sim.t, i] <- 0.
  ...
  #
  # comment in if you want to do some plotting
	if(single.sim) {
