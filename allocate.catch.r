allocate.catch <- function(fishing.option, historical.catch, init.tot.demand, init.krill.biomass, option5.list, tt.density, relQdev1, seals.params.list, pengs.params.list, earmarks, B0, actual.gamma, wbar.vector, catch.setup, ntimes, nssmus, start.fishing, obs.errors, obs.bias){
  #
  # George Watters - code last edited by GW 23 June 2006
  # Edited Aug 2018 - ESK for FBM 
  #
  ...
  # ESK UPDATE HERE FOR KRILL DENSITY - AUG 2018
  if(fishing.option == 3) {
    # option.string <- "krill standing stock"
    option.string <- "krill density"
...
### NEW CODE FOR FBM ###
  if(fishing.option == 7) {
    option.string <- "penguin monitoring"
    # Get proportions of previous catch in proportions - NB do not use this for George alpha option
    catch.x <- historical.catch/sum(historical.catch)
    # Next add bias and observation error to changes in penguins
    tt <- init.tot.demand
    # tt <- obs.bias * init.tot.demand * exp(obs.errors)
### Opn 1: Catch is scaled by change in penguins, either directly or via current proportions
    # (make sure main code is pengs2/pengs1)
    # tt <- historical.catch*tt
### Opn 2: Get proportion of total change in penguins for each area,
    # main code needs to be pengs2-pengs1
    # create same increase or decrease in catch depending on direction of change in pengs
    # tt <- tt/sum(abs(tt))
    # tt <- catch.x*(1+tt)
    # or don't x by catch at all - 
    # tt <- 1+tt
### Opn 3: initial catch scaled by change in pengs (main code is pengs2/pengs1)
    # I think this may end up being the same as Opn 1
    # tt <- catch.x*tt
### George alpha option
    alpha <- 1.0  # can update this later to be a code input
    # This is to leave out the SSMUs we know don't have penguins in them (have to think abt how to automate this)
    tt.with <- c(tt[2:8], tt[10:12], tt[14:15])
    min_pengs <- min(tt.with, na.rm = TRUE)
    tt <- tt + (alpha*abs(min_pengs))
    #
    allocation <- tt/sum(tt)
    area.earmark <- (B0 * actual.gamma * allocation)/wbar.vector
    earmarks[start.fishing:ntimes,  ] <- matrix(rep(area.earmark, each = ntimes - start.fishing + 1), nrow = ntimes - start.fishing + 1, ncol = nssmus)*catch.setup
