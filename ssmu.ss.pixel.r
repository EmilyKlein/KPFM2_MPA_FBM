ssmu.ss.pixel<-function(param.list, saved.state = NULL, GGP=NULL, nyears = 38, start.fishing.yr =1, stop.fishing.yr = NULL, rescale.catch = FALSE, random.Rkrill = FALSE, sd.krill.Rdev = 0.4,
  ...
  safe.mode = TRUE, annual.plots = TRUE, annual.connector = "default", Rylimits =	c(0, 3), Nylimits = c(0, 3), Cylimits = c(0, 3), page.layout = c(4, 4), point.size = 0.001, single.sim = TRUE, suppress.messages=FALSE, ss.seed = 123, kitchen.sink=FALSE)
{
  ...
  # This code is updated for pixelated input - ESK 2017
	#
	cl <- match.call()
	#
  ...
  if(is.null(param.list$ORIG.SSMU)){
    stop("Input must be updated via pixel.foosa. This input should be analyzed by the original ssmu.ss code")
  }
...
### Basic set up is to here###
  ...
  wbar.matrix<-matrix(0, nrow=ntimes, ncol=nareas)  ## UPDATED HERE FOR PIXELATED FOOSA - ESK JUNE 2017
	#
    ...
    # loop over all areas to get mortalities and "competition reduction fractions"
      ...
      # now compute the catch if fishing.option=5
		...
		#
		# now loop over areas to get recruitment, move krill, and project abundance
		for(i in 1:nssmus){
			# get parameters
			krill.params <- krill.params.list[[i]]
			seals.params <- seals.params.list[[i]]
  ...
  ss.out <- list(call = cl, setup=list(ntimes=ntimes, nseasons=nseasons, nareas=nareas, nssmus=nssmus, ntubs=ntubs, orig.SSMU=orig.ssmu, 
                 pixel.list=pixel.list,init.season=init.season, start.fishing=start.fishing, stop.fishing=stop.fishing, 
                 reassessment.times=reassessment.times),N=list(krill=krill, seals=seals, pengs=pengs, whales=whales, fish=fish), 
                 R=list(R=list(krill=Rkrill, seals=Rseals, pengs=Rpengs, whales=Rwhales, fish=Rfish), maxRage=list(krill=krill.Rage, seals=seals.maxRage, pengs=pengs.maxRage, whales=whales.maxRage, fish=fish.maxRage)),
                 Q=list(demand=list(seals=Qseals, pengs=Qpengs, whales=Qwhales, fish=Qfish), consumption=list(seals=Qseals.performance, pengs=Qpengs.performance, whales=Qwhales.performance, fish=Qfish.performance)), 
                 fishery=list(allocation=earmarks, catch=catch, threshold.violations=threshold.violations), wbar=wbar.matrix)
  #
  # Can comment in if you want to do some plotting
	if(single.sim) {
    plot.ss.pixel.N(ss.out, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Nylimits = Nylimits)
    plot.ss.pixel.R(ss.out, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Rylimits = Rylimits)
    plot.ss.pixel.C(ss.out, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Cylimits = Cylimits, point.size = point.size)
	}
	#
	# return the results
