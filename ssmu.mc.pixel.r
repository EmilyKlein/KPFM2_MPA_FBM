ssmu.mc.pixel<-function(param.list, saved.state = NULL, ggp=NULL,ntrials = 2, quants=c(0.2, 0.5, 0.8), plot.now = TRUE,
	...
	point.size = 0.001, mc.seed = 123, aggregate=FALSE)
{
	# George Watters - lasy edited July 2014 by JTH 
  # Code updated by ESK in 2017 to allow for pixelated input.
  #
  cl <- match.call()
  set.seed(mc.seed)
	#
  ...
  nareas <- dim(param.list$V.ARRAY)[1]
	...
	orig.ssmu <- param.list$ORIG.SSMU
	pixel.list <- param.list$PIXEL.LIST
	#
  ...
  wbars<-array(0, dim=c(ntimes, nareas, ntrials))
    ...
    # allow the user to input a matrix of bathtub abundances
		...
		#
	  #CALENDAR: tt <- ssmu.ss.pixel(param.list = param.list, GGP=ggp, saved.state = saved.state, nyears = nyears, start.fishing.yr = start.fishing.yr, stop.fishing.yr = stop.fishing.yr, rescale.catch = rescale.catch, random.Rkrill = random.Rkrill, sd.krill.Rdev = sd.krill.Rdev,
    tt <- ssmu.ss.pixel(param.list = param.list, GGP=ggp, nyears = nyears, start.fishing.yr = start.fishing.yr, stop.fishing.yr = stop.fishing.yr, rescale.catch = rescale.catch, random.Rkrill = random.Rkrill, sd.krill.Rdev = sd.krill.Rdev,
	...
	#
	mc.out <- list(call = cl, setup=list(ntrials = ntrials, ntimes=ntimes, nseasons=param.list$NSEASONS, 
	               nareas=nareas, nssmus=nssmus, ntubs=param.list$NTUBS, orig.SSMU=orig.ssmu, pixel.list=pixel.list,
	               init.season=param.list$INIT.SEASON, start.fishing=start.fishing, stop.fishing=stop.fishing), 
	               N=list(krill=krill, seals=seals, pengs=pengs, whales=whales, fish=fish), 
	               R=list(R=list(krill=Rkrill, seals=Rseals, pengs=Rpengs, whales=Rwhales, fish=Rfish),
                 maxRage=list(krill=krill.Rage, seals=seals.maxRage, pengs=pengs.maxRage, whales=whales.maxRage, fish=fish.maxRage)),
	               Q=list(demand=list(seals=Qseals, pengs=Qpengs, whales=Qwhales, fish=Qfish), 
                 consumption=list(seals=Qseals.performance, pengs=Qpengs.performance, whales=Qwhales.performance, fish=Qfish.performance)), 
	               fishery=list(allocation=earmarks, catch=catch, threshold.violations=threshold.violations), wbar=wbars)
  # 
	# If the output should be reorganized and plotted by SSMU - ESK 2017
	if(aggregate) {   
 	  mc.out <- aggregate.pixel(mc.out, mc=TRUE)
	  #
	  if(plot.now) {
	    ...
	    # plot krill
	    plot.mc.pixel.N(mc.out, spp.string = "krill", quants = quants, plot.trials = plot.trials, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Nylimits = Nylimits, color.tags = NULL)
	    plot.mc.pixel.R(mc.out, spp.string = "krill", quants = quants, plot.trials = plot.trials, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Rylimits = Rylimits, color.tags = NULL)
	    #
	    # plot seals
	    plot.mc.pixel.N(mc.out, spp.string = "seals", quants = quants, plot.trials = plot.trials, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Nylimits = Nylimits, color.tags = NULL)
	    plot.mc.pixel.R(mc.out, spp.string = "seals", quants = quants, plot.trials = plot.trials, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Rylimits = Rylimits, color.tags = NULL)
	    #
	    # plot pengs
	    plot.mc.pixel.N(mc.out, spp.string = "pengs", quants = quants, plot.trials = plot.trials, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Nylimits = Nylimits, color.tags = NULL)
	    plot.mc.pixel.R(mc.out, spp.string = "pengs", quants = quants, plot.trials = plot.trials, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Rylimits = Rylimits, color.tags = NULL)
	    #
	    # plot whales
	    plot.mc.pixel.N(mc.out, spp.string = "whales", quants = quants, plot.trials = plot.trials, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Nylimits = Nylimits, color.tags = NULL)
	    plot.mc.pixel.R(mc.out, spp.string = "whales", quants = quants, plot.trials = plot.trials, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Rylimits = Rylimits, color.tags = NULL)
	    #
	    # plot fish
	    plot.mc.pixel.N(mc.out, spp.string = "fish", quants = quants, plot.trials = plot.trials, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Nylimits = Nylimits, color.tags = NULL)
	    plot.mc.pixel.R(mc.out, spp.string = "fish", quants = quants, plot.trials = plot.trials, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Rylimits = Rylimits, color.tags = NULL)
	    #
	    # plot catch
	    plot.mc.pixel.C(mc.out, quants = quants, plot.trials = plot.trials, annual.plots = annual.plots, connector = annual.connector, page.layout = page.layout, Cylimits = Cylimits, color.tags = NULL)
	  }
	}else{
	  if(plot.now) {
	    if(!is.null(fishing.option)){
