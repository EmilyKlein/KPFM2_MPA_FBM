...
###############################################################################################################
run.mcsims <- function(fishing.options, input.objects, GGP.in, ntrials, nyears, start.fishing.yr, stop.fishing.yr, reassessment.interval.yr,
                        actual.gamma, sd.krill.Rdev, parameter.path, output.file.path, dens.thresholds) {
	...
	for (fishing.option in fishing.options) {	
		...
		for(b in seq(input.objects)){ 
			...
			if(!is.null(dens.thresholds)){p1$THRESHOLD.DENSITY <- rep(dens.thresholds, 15)}
...
############ For CM 51-07 re-analysis, needed to update a bunch of stuff - ESK, Sept 2016  ############
			...
			if (fishing.option==0) {
				...
				if (OVERWRITE==TRUE | file.exists(tmpfile)==FALSE) {
					...
					# assign(tmp, ssmu.mc(p2, saved.state=tt.ss, ggp=GGP.in, safe.mode=FALSE, ntrials=ntrials, nyears=nyears, fishing.option=NULL, sd.krill.Rdev=sd.krill.Rdev, plot.now=FALSE))
					assign(tmp, ssmu.mc(p2, saved.state=tt.ss, ggp=GGP.in, safe.mode=FALSE, ntrials=ntrials, nyears=nyears, fishing.option=NULL, sd.krill.Rdev=sd.krill.Rdev, plot.now=FALSE))
				...
				if (!length(mc.files)==0) { #if there's anything to process ...
					for(k in seq(mc.objects)) {#seq(gamma.fractions)){
						...
						## turned off 2018 ESK - fishing option 7 about FBM now... !
						# if (fishing.option == 7) {		
						...
						# 	
						# } else {
							# assign(mc.objects[k], ssmu.mc(p2, saved.state=tt.ss, safe.mode=FALSE, ggp=GGP.in, ntrials=ntrials, nyears=nyears, fishing.option=fishing.option,
							#                               start.fishing.yr=start.fishing.yr, stop.fishing.yr=stop.fishing.yr, actual.gamma=actual.gamma*gamma.fraction,
							#                               reassessment.interval.yr = reassessment.interval.yr, sd.krill.Rdev=sd.krill.Rdev,plot.now=FALSE))
							assign(mc.objects[k], ssmu.mc(p2, saved.state=tt.ss, safe.mode=FALSE, ggp=GGP.in, ntrials=ntrials, nyears=nyears, fishing.option=fishing.option,
							                                  start.fishing.yr=start.fishing.yr, stop.fishing.yr=stop.fishing.yr, actual.gamma=actual.gamma*gamma.fraction,
							                                  reassessment.interval.yr = reassessment.interval.yr, sd.krill.Rdev=sd.krill.Rdev,plot.now=FALSE))
							# # }
